...
//! how a threshold is applied on top (ADR-0012).
//!
...
//!   RRN_REGEN=1 cargo test -p rrn-crypto --test it cross_platform_multisign
...
fn build_fixture() -> Fixture {
    ...
    Fixture {
        comment: "Cross-platform multisig verification vectors \
...
fn committed_fixture_is_in_sync() {
    ...
    assert_eq!(
        ...
        "fixture drift — regenerate with RRN_REGEN=1 cargo test -p rrn-crypto \
         --test it cross_platform_multisign, then copy the JSON into the mobile repo"
    );
}
