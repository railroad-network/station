//! Cross-platform `SignedPayload<Listing>` vectors.
//!
...
//!   RRN_REGEN=1 cargo test -p rrn-station --test it cross_platform_listing
...
fn build_fixture() -> Fixture {
    Fixture {
        comment: "Cross-platform SignedPayload<Listing> vectors. Generated by \
...
fn committed_fixture_is_in_sync() {
    ...
    assert_eq!(
        ...
        "fixture drift — regenerate with RRN_REGEN=1 cargo test -p rrn-station \
         --test it cross_platform_listing, then copy the JSON into the mobile repo"
    );
}
