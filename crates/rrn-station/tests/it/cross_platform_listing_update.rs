//! Cross-platform `SignedPayload<ListingUpdated>` vectors.
//!
...
//!   RRN_REGEN=1 cargo test -p rrn-station --test it cross_platform_listing_update
...
fn build_fixture() -> Fixture {
    Fixture {
        comment: "Cross-platform SignedPayload<ListingUpdated> vectors. \
...
fn committed_fixture_is_in_sync() {
    ...
    assert_eq!(
        ...
        "fixture drift — regenerate with RRN_REGEN=1 cargo test -p rrn-station \
         --test it cross_platform_listing_update, then copy the JSON into the mobile repo"
    );
}
