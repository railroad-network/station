...
//!   a connectivity event, exactly like an unreachable peer.
...
//!
//! This module delivers the supervisor and the integration spike. The
//! `FrameTransport` impl over the sidecar and announce-budget tuning come with the
//! Reticulum transport; the RNode/LoRa interface template comes later.
...
/// Spike pins — `rns` 1.5.2 (PyPI, 2026-08-29) with `lxmf` 1.1.1;
...
/// block so an operator can read the sidecar's posture at a glance.
...
pub struct SidecarConfig {
    ...
    /// The physical RNode LoRa radio interface (`[lora.rnode]`) to template, if
    /// configured. Absent → the generated config carries a commented example
    /// instead of an active radio.
    pub rnode: Option<crate::config::RNodeSection>,
...
pub async fn supervise(
    ...
    if let Err(e) = write_config_if_absent(
        &cfg.config_dir,
        cfg.tcp_listen.as_deref(),
        &cfg.tcp_peers,
        cfg.rnode.as_ref(),
    ) {
        let reason = format!(
...
pub fn write_config_if_absent(
    ...
    tcp_peers: &[String],
    rnode: Option<&crate::config::RNodeSection>,
) -> std::io::Result<PathBuf> {
    std::fs::create_dir_all(dir)?;
    let path = dir.join(RETICULUM_CONFIG_FILE);
    if !path.exists() {
        std::fs::write(&path, render_reticulum_config(tcp_listen, tcp_peers, rnode))?;
    }
    Ok(path)
}

/// Renders a minimal Reticulum config (RNS ConfigObj format) with the TCP
/// interfaces the station names — a server stanza for `tcp_listen` and one client
/// stanza per `tcp_peers` entry — plus, when `rnode` is set, an active
/// `RNodeInterface` stanza for the physical LoRa radio (ADR-0013). When `rnode` is
/// absent the config carries a **commented** RNode example and a docs pointer
/// instead, so a station never transmits on a frequency or power nobody chose.
///
...
/// probe (`rnstatus`) and the Reticulum transport will speak to.
pub fn render_reticulum_config(
    tcp_listen: Option<&str>,
    tcp_peers: &[String],
    rnode: Option<&crate::config::RNodeSection>,
) -> String {
    ...
    s.push_str(
        "# Generated by the Railroad Network station (ADR-0013).\n\
    ...
    // local control socket that `rnstatus` and the status probe query.
    s.push_str(
        "[reticulum]\n  \
    ...
    match rnode {
        Some(r) => {
            // Physical LoRa radio (ADR-0013). RNS RNodeInterface keys, verified
            // against the RNS 1.5 manual (interfaces.html, "RNode LoRa Interface").
            // frequency/txpower carry no defaults in [lora.rnode] — an active radio
            // is always a deliberate, region-aware operator choice.
            s.push_str(&format!(
        ...
        None => {
            // No radio configured: leave a commented example, never an active
            // interface. Frequency and TX power MUST be chosen for the operator's
            // region — see docs/lora-radio-bringup.md and its compliance table.
            s.push_str(
                "  # No LoRa radio configured. To carry station traffic over a\n  \
                 # radio, set [lora.rnode] in the station config (frequency_hz and\n  \
                 # tx_power_dbm have NO defaults — you must choose values that are\n  \
                 # legal for your region; see docs/lora-radio-bringup.md and its\n  \
                 # regional compliance table), or add a stanza like the one below by\n  \
                 # hand. Every node on one LoRa network must share the same\n  \
                 # frequency, bandwidth, and spreading factor.\n  \
                 #  [[RNode LoRa Interface]]\n  \
                 #    type = RNodeInterface\n  \
                 #    interface_enabled = True\n  \
                 #    port = /dev/ttyACM0\n  \
...
mod tests {
    ...
    fn rnode_eu868() -> crate::config::RNodeSection {
        crate::config::RNodeSection {
    ...
    fn config_template_omits_absent_interfaces() {
        let cfg = render_reticulum_config(None, &[], None);
        assert!(cfg.contains("[interfaces]"));
    ...
    #[test]
    fn config_template_renders_active_rnode_interface_when_configured() {
        let cfg = render_reticulum_config(None, &[], Some(&rnode_eu868()));
        assert!(cfg.contains("[[RNode LoRa Interface]]"));
        assert!(cfg.contains("type = RNodeInterface"));
        assert!(cfg.contains("interface_enabled = True"));
        assert!(cfg.contains("port = /dev/ttyACM0"));
        assert!(cfg.contains("frequency = 867200000"));
        assert!(cfg.contains("bandwidth = 125000"));
        assert!(cfg.contains("txpower = 7"));
        assert!(cfg.contains("spreadingfactor = 8"));
        assert!(cfg.contains("codingrate = 5"));
        // The configured stanza is live, not left commented out.
        assert!(!cfg.contains("#    type = RNodeInterface"));
    }

    #[test]
    fn config_template_leaves_commented_rnode_example_when_absent() {
        let cfg = render_reticulum_config(Some("0.0.0.0:4242"), &[], None);
        // No active radio: the RNodeInterface appears only inside a comment, with a
        // pointer to the bring-up runbook so the operator makes a deliberate choice.
        assert!(cfg.contains("docs/lora-radio-bringup.md"));
        assert!(cfg.contains("#    type = RNodeInterface"));
        assert!(!cfg.contains("\n    type = RNodeInterface"));
        // No line enables a radio interface — every RNodeInterface mention is a
        // comment.
        for line in cfg.lines() {
            if line.contains("RNodeInterface") {
                assert!(
                    line.trim_start().starts_with('#'),
                    "uncommented RNodeInterface line leaked: {line:?}"
                );
            }
        }
    }
    ...
    fn write_config_is_idempotent_and_never_clobbers() {
        ...
        let path = write_config_if_absent(&cfgdir, Some("0.0.0.0:4242"), &[], None).unwrap();
        assert!(path.exists());
        ...
        let again = write_config_if_absent(&cfgdir, Some("0.0.0.0:4242"), &[], None).unwrap();
