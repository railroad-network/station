//! Cross-platform `SignedPayload<TransactionProposal>` vectors.
//!
...
/// The optional marketplace listing link: present on some indices,
...
fn build_fixture() -> Fixture {
    Fixture {
        comment: "Cross-platform SignedPayload<TransactionProposal> vectors. \
...
fn committed_fixture_is_in_sync() {
    ...
    assert_eq!(
        ...
        "fixture drift — regenerate with RRN_REGEN=1 cargo test -p rrn-ledger \
         --test it cross_platform_signed_payload, then copy the JSON into the mobile repo"
    );
}
