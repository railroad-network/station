...
//!
//! ```sh
//! RRN_REGEN=1 cargo test -p rrn-dispute --test it cross_platform_equivocation_jury
//! ```
...
fn committed_fixture_is_in_sync() {
    ...
    assert_eq!(
        ...
        "fixture drift — regenerate with RRN_REGEN=1 cargo test -p rrn-dispute \
         --test it cross_platform_equivocation_jury, then copy the JSON into the mobile repo"
    );
}
