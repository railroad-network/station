//! Consolidated cross-platform FFI invariants.
//!
//! The address, signing, and wallet-file fixtures each locked one slice of the
//! mobile/station contract (address parity, signing parity, wallet-file
//! parity). This test
...
//! a `canonical_bytes` FFI surface which does not exist yet — it is a later
...
//! a "same struct → same bytes" check. The dcbor invariant lands with that
//! later surface.
//!
...
fn build_fixture() -> Fixture {
    ...
    Fixture {
        comment: "Consolidated cross-platform FFI invariants. Generated by \
            rrn-identity/tests/ffi_invariants.rs. Rolls the address (ADR-0003), signing \
            (ADR-0001), and wallet-file invariants into one suite and adds \
            ...
            deferred to a later FFI surface. Regenerate with RRN_REGEN=1."
            .to_string(),
...
fn deterministic_sections_match_generator() {
    ...
    assert_eq!(
        ...
        "fixture drift — regenerate with RRN_REGEN=1 cargo test -p rrn-identity \
         --test it ffi_invariants, then copy the JSON into the mobile repo"
    );
}
