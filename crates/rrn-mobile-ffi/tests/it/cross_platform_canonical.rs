//! Cross-platform canonical-CBOR vectors for the tagged-value model.
//!
...
//! errors. This is the generic half of the canonical-CBOR coverage; the `SignedPayload<TransactionProposal>`
...
fn build_fixture() -> Fixture {
    ...
    Fixture {
        comment: "Cross-platform canonical-CBOR vectors. Generated by \
...
fn committed_fixture_is_in_sync() {
    ...
    assert_eq!(
        ...
        "fixture drift — regenerate with RRN_REGEN=1 cargo test -p rrn-mobile-ffi \
         --test it cross_platform_canonical, then copy the JSON into the mobile repo"
    );
}
