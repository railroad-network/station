...
### Phase 2 — Single-Community Resilience (Months 11-16)

Make one community's system work when infrastructure fails. This is Railroad Network's
core differentiator, and it comes before federation deliberately: delay-tolerant sync and
federation gossip are largely the same machinery, so proving offline-first,
store-and-forward semantics first means the federation protocol inherits
partition-tolerance as its default posture instead of having it retrofitted. It also
serves the Phase 1 early adopters — rural communities with weak connectivity, mutual aid
...
Connect communities together. This is where the Railroad metaphor becomes real. Because
it builds on Phase 2, the federation protocol assumes disconnection as the default
condition, and the inter-community pieces of the resilience story land here.
...
**Exit criteria:** Three communities actively trading. At least one inter-community
dispute resolved. Predictive matching surfaces at least one trade that would not have
happened through manual search. And the hardening test: simulated 72-hour full
connectivity loss across all 3 communities — all transactions reconcile, no credits
lost, no ledger forks.
...
| **Phase 2 — Single-community resilience** | Still mostly invisible. Offline and radio infrastructure for one community fits the disaster-preparedness / mutual-aid framing and is legally defensible at this scale — though building infrastructure to operate outside state-controlled networks is what later invites national security framing. |
| **Phase 3 — Federated credit system** | Attracts attention. A federated credit system moving value across communities looks like an unlicensed payment network to financial regulators — and because it now arrives with resilience transports already built in, national security framing can enter here too. |
