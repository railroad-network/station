...
member rows at once is `crates/rrn-station/tests/outage_72h.rs` (see
...
| **Self-sign an Overturn** to lift the penalty | The verdict kind is refused on DTN (`UnroutableKind`) and has no RPC; reputation and the snapshot honor an Overturn only from the equivocation record's own signer. **Residual:** that is a record-author gate, not a pin to the community key (threat model, `rrn-ledger` "Equivocation jury"). | Inert today; needs the station key threaded into replay once foreign records are admitted. | Courier a member-signed `rrn.credit.equivocation_verdict`. |
...
| **Forge a settlement / certificate / contract charge** | Station-signed by design (ADR-0005). **Residual:** ledger station-signed records are replay-trusted with **no signer pin** (threat model, `rrn-governance` residual list item 1); governance attestations *are* pinned (`crates/rrn-governance/tests/station_signer_pinning.rs`). Only the station writes the log, so the vector is a hostile gossip peer, mitigated by "no untrusted gossip peers" in the pilot. | A forged settlement injected via `append_raw` would be believed. | Configure a hostile gossip peer serving a forged `SettlementRecord`. |
...
| **Forge a governance attestation** — window, enactment, emergency activation/refusal/anchor | Replay pins the envelope signer of all five station-signed governance kinds to the community station key and skips any other (`window::window_and_seq_of`, `statute::enacted_statutes`, `emergency::derive_emergencies`); `crates/rrn-governance/tests/station_signer_pinning.rs`. | Forged records are invisible; derivation completes. | Inject via gossip (tests `a_forged_activation_does_not_enter_the_timeline`, `a_forged_refusal_does_not_kill_a_declaration`). |
...
| **Gossip a forged or ungated record** to a station that peers with you | `append_raw` re-verifies signature and content hash and re-chains locally. **Residual (pre-existing):** the gossip pull path admits any signature-valid record with no front-door gate (`do_append_entries`) — a member `emergency_cosign` can land markerless, though it is not activatable (ADR-0027 fail-closed). Pilot mitigation: `[peers] list = []`. | A well-signed but ungated record lands on a replica. | Configure yourself as a peer. |
...
4. Overturn gate is a record-author gate, not a community-key pin — threat model `rrn-ledger`, "Equivocation jury".
...
7. Gossip ingest bypasses the front door — threat model `rrn-governance`, residual; ADR-0027 fail-closed covers activation only.
