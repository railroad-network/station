...
*Implemented in M1.1 (T1.1.3–T1.1.5).* The mobile app performs **no**
cryptography of its own: address parsing, signing, and the `.rrnwallet` file
format all cross into the pure-Rust `rrn-crypto` / `rrn-identity` code through a
single, narrow uniffi surface (`rrn-mobile-ffi`, ADR-0007). The TS wrappers
(`crypto/address.ts`, `crypto/sign.ts`, `wallet/Wallet.ts`) marshal bytes and
strings; they hold no keys and branch on no crypto. This subsection covers that
boundary; the OS storage it feeds is the `SecureStore` subsection above.
...
- **No divergent second implementation.** *Threat:* a reimplemented bech32 /
  ...
  *Mitigation:* there is exactly one implementation (Rust), reached from both
  ...
  byte-identical signatures), and `cross_platform_wallet.json` (T1.1.5, each
  station-sealed `.rrnwallet` decrypts to its recorded identity). *Residual risk:*
  the fixtures cover the invariants enumerated, not the entire input space; the
  Rust property tests (T1.1.6) widen that coverage.
